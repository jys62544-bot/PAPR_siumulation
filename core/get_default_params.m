...
% 在每个脚本中首先调用此函数，然后根据需要覆盖特定字段。
    ...
    params.mod_type  = '16QAM';   % 非自适应模式下的统一调制方式
    params.L         = 4;         % 过采样倍数
    params.N_sym     = 10000;     % 仿真符号数
    params.snr_range = 0:2:30;    % 信噪比范围（dB）

    % 自适应调制参数
    % 当 adaptive=true 时，使用多径信道 + 逐子载波自适应调制；
    % 当 adaptive=false 时，使用 AWGN 信道 + 统一调制方式（mod_type）
    params.adaptive       = true;          % 启用逐子载波自适应调制
